...
int num_of_moves = sizeof(pre_moves) / sizeof(pre_moves[0]);
    
	int moves[num_of_moves][2];
		for (int i = 0; i < num_of_moves; i++) {
		    moves[i][0] = pre_moves[i][0];
		    moves[i][1] = pre_moves[i][1];
		}
		int move_index = 0;
    while (!game_over && move_index < num_of_moves) { 
        system("cls");
        print_board();
